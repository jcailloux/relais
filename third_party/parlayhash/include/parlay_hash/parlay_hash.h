...
struct parlay_hash {
  ...
  // accessor for current table version (used by unordered_map wrappers)
  table_version* current_table() { return current_table_version.load(); }
  ...
  struct IndirectEntries {
	...
	return (((k.second >> 48) == (((size_t) ptr) >> 48)) &&
      ...
      static const K& user_key(const Key& k) { return *k.first; }
  ...
  struct DirectEntries {
	...
	size_t hash_val;
      ...
      static unsigned long hash(const Key& k) {
	return k.hash_val;}
      bool equal(const Key& k) const { return KeyEqual{}(DataS::get_key(data), k.key); }
      static Key make_key(const K& k) {return Key{k, rehash<Hash>{}(Hash{}(k))};}
      Key get_key() const {return make_key(DataS::get_key(data));}
      static const K& user_key(const Key& k) { return k.key; }
