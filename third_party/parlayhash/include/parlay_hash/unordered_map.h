...
namespace parlay {
  ...
  struct unordered_map_internal {
    ...
    using hashed_key = typename Entry::Key;
    ...
    static hashed_key make_key(const K& key) { return Entry::make_key(key); }
    static size_t get_hash(const hashed_key& k) { return Entry::hash(k); }
    ...
    auto Insert(const hashed_key& k, const V& value) -> std::optional<mapped_type>
    {
    ...
    template <typename F>
    auto Upsert(const hashed_key& k, const F& f) -> std::optional<mapped_type>
    {
      const K& key = Entry::user_key(k);
      auto g = [&] (const Entry& e) {return get_value(e.get_entry());};
      auto constr = [&] (const std::optional<Entry>& e) -> Entry {
    ...
    template <typename F>
    auto Insert(const hashed_key& k, const V& value, const F& f)
    {
      const K& key = Entry::user_key(k);
      auto g = [&] (const Entry& e) {return f(e.get_entry());};
      return m.Insert(k, [&] {return entries_.make_entry(k, value_type(key, value));}, g);
    ...
    size_t erase(const K& k) { return m.erase(Entry::make_key(k)); }

    // Iterate entries in a single bucket. Callback: f(const K&, V&)
    // Caller must hold EpochGuard. Safe during resize (follows forwarded buckets).
    template<typename F>
    void for_each_bucket(long i, F&& f) {
        auto* tv = m.current_table();
        map::for_each_bucket_rec(tv, i, [&](const auto& entry) {
            auto& [k, v] = entry.get_entry();
            f(k, v);
        });
    }

    // Number of buckets in current table version.
    long num_buckets() { return m.current_table()->size; }

    // Bucket index for a user key.
    long bucket_for_key(const K& key) {
