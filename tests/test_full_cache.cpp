          ...
          "[integration][db][full-cache][item]")
{
    ...
    SECTION("[findById] L1 miss with L2 hit promotes to L1") {
          ...
          "[integration][db][full-cache][ttl]")
{
    ...
    SECTION("[ttl] L1 expires but L2 still serves") {
        ...
        sync(ShortL1BothItemRepo::findById(id));
        ...
        auto item = sync(ShortL1BothItemRepo::findById(id));
    ...
    SECTION("[ttl] L2 repopulates L1 after L1 expiration") {
        ...
        auto cached = sync(ShortL1BothItemRepo::findById(id));
          ...
          "[integration][db][full-cache][write-through]")
{
    ...
    SECTION("[write-through] update populates L1 immediately") {
        ...
        auto item = sync(WriteThroughBothItemRepo::findById(id));
          ...
          "[integration][db][full-cache][cross-inv]")
{
    ...
    SECTION("[cross-inv] purchase create invalidates user in both L1 and L2") {
        ...
        sync(FullCacheInvUserRepo::findById(userId));
        ...
        auto user = sync(FullCacheInvUserRepo::findById(userId));
        REQUIRE(user->balance == 200);
    }
    ...
    SECTION("[cross-inv] FK change invalidates old and new user in both layers") {
        ...
        sync(FullCacheInvUserRepo::findById(user1Id));
        sync(FullCacheInvUserRepo::findById(user2Id));
        ...
        auto u1 = sync(FullCacheInvUserRepo::findById(user1Id));
        auto u2 = sync(FullCacheInvUserRepo::findById(user2Id));
...
TEST_CASE("FullCache - findByIdAsJson at L1+L2",
