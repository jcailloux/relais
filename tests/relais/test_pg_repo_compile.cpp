 ...
 *
 * Compile-time and structural tests for PgRepo.
 * Verifies that:
 *   - PgRepo instantiates with all entity types
...
using jcailloux::relais::entity::fieldColumnName;
using jcailloux::relais::entity::fieldValue;

// =========================================================================
// Instantiate PgRepo with each entity type to verify compilation.
// These are direct PgRepo instantiations (not Repo<>) to avoid
// depending on the full mixin chain (RedisRepo, LocalRepo, etc.) which
...
TEST_CASE("FieldUpdate utilities", "[base_repo][field_update]") {
    ...
    SECTION("set() creates FieldUpdate with correct value") {
        auto update = set<Field::name>(std::string("test_name"));
        REQUIRE(update.value == "test_name");
    }

    SECTION("set() with integer value") {
        auto update = set<Field::value>(42);
        REQUIRE(update.value == 42);
    }

    SECTION("fieldColumnName returns quoted column name") {
        auto update = set<Field::name>(std::string("test"));
        auto col = fieldColumnName<Traits>(update);
        REQUIRE(col == "\"name\"");
    }

    SECTION("fieldColumnName for value field") {
        auto update = set<Field::value>(0);
        auto col = fieldColumnName<Traits>(update);
        REQUIRE(col == "\"value\"");
    }
    ...
    SECTION("fieldValue for boolean field") {
        auto update = set<Field::is_active>(true);
        auto val = fieldValue<Traits>(update);
        REQUIRE(val == true);
    }
}
...
TEST_CASE("FieldUpdate with nullable fields", "[base_repo][field_update]") {
    ...
    SECTION("setNull for nullable field compiles and returns nullptr") {
        auto update = setNull<Field::discount>();
        auto val = fieldValue<Traits>(update);
        REQUIRE(val == nullptr);
    }
