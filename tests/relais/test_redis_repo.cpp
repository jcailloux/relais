...
io::Task<int64_t> redisTTL(const std::string& key) {
    auto result = co_await jcailloux::relais::PgProvider::redis("TTL", key);
    co_return result.asInteger();
}

io::Task<int64_t> redisExists(const std::string& key) {
    auto result = co_await jcailloux::relais::PgProvider::redis("EXISTS", key);
    co_return result.asInteger();
}

io::Task<int64_t> redisSCard(const std::string& key) {
    auto result = co_await jcailloux::relais::PgProvider::redis("SCARD", key);
    co_return result.asInteger();
}

io::Task<bool> redisSetContains(const std::string& setKey, const std::string& member) {
    auto result = co_await jcailloux::relais::PgProvider::redis("SISMEMBER", setKey, member);
...
namespace list = jcailloux::relais::list;
...
public:
    ...
    static io::Task<size_t> invalidateAllListGroups() {
        ...
        auto result = co_await jcailloux::relais::PgProvider::redis("KEYS", pattern);
...
using ArticleListTarget = jcailloux::relais::ListInvalidationTarget<ArticleGroupKey>;
