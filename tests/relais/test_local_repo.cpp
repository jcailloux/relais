 ...
 *
 * Tests for LocalRepo (L1 - RAM caching on top of database).
 * Uses L1 configurations that resolve to LocalRepo via Repo<>.
 *
...
using L1MockTarget = jcailloux::relais::ListInvalidationTarget<L1MockGroupKey>;
...
TEST_CASE("LocalRepo<TestItem> - find",
...
TEST_CASE("LocalRepo<TestItem> - insert",
          "[integration][db][cached][item]")
{
    TransactionGuard tx;
...
TEST_CASE("LocalRepo<TestItem> - update",
          "[integration][db][cached][item]")
{
    TransactionGuard tx;
...
TEST_CASE("LocalRepo<TestItem> - erase",
          "[integration][db][cached][item]")
{
    TransactionGuard tx;
...
TEST_CASE("LocalRepo<TestItem> - explicit invalidation",
...
TEST_CASE("LocalRepo - ShortTTL config",
...
TEST_CASE("LocalRepo - WriteThrough config",
...
TEST_CASE("LocalRepo - FewChunks config",
...
TEST_CASE("LocalRepo - cross-invalidation Purchase → User",
...
TEST_CASE("LocalRepo - custom cross-invalidation via resolver",
...
TEST_CASE("LocalRepo - entity to ListDescriptor cross-invalidation",
...
TEST_CASE("LocalRepo - InvalidateListVia per-page resolver",
...
TEST_CASE("LocalRepo - InvalidateListVia per-group resolver",
...
TEST_CASE("LocalRepo - InvalidateListVia full pattern resolver",
          "[integration][db][cached][list-resolver][list-granularity]")
{
    TransactionGuard tx;
    L1MockArticleListRepo::reset();
...
TEST_CASE("LocalRepo - InvalidateListVia mixed granularity",
          "[integration][db][cached][list-resolver][list-granularity]")
{
    TransactionGuard tx;
    L1MockArticleListRepo::reset();
...
TEST_CASE("LocalRepo<TestUser> - binary caching",
...
TEST_CASE("LocalRepo<TestUser> - patch",
...
TEST_CASE("LocalRepo - read-only as cross-invalidation target",
